...
/*getorder:给定请求的页大小n，返回合适的order
           有2的order-1次 < n <= 2的order次
*/ 
...
buddy_free_pages_sub(struct Page *base, size_t order) {
	...
	/* LAB2 PROJ5:  
	 *
	 * you can calculate the buddy's index
	 * but you still need to check permission
	 *
	...
	while (order < MAX_ORDER) {
        ...
        struct Page *bp = idx2page(zone_num, buddy_idx);
        // check permission
		if (!page_is_buddy(bp, order, zone_num)) {
            break;
        }
        nr_free(order) --;
		// remove
        ...
        ClearPageProperty(bp);
        page_idx &= buddy_idx;
        order ++;
    }
	
	// add final buddy and set properties
