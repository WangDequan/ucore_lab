...
sem_t sem_read, 	//the semophore is set up for the waiting readers
	sem_write, 	//the semophore is set up for the waiting writers
	sem_console;	//the semophore is set up for function scprintf
sem_t sem_x, 		//the semophore is set up for mutually exclusive access of pCountR
	sem_y, 		//the semophore is set up for mutually exclusive access of pCountW
	sem_z;
int *pCountR, 		//recording the number of waiting readers
	*pCountW;	//recording the number of waiting writers
...
int
check_sem_value_sub(sem_t sem_id, int value) {
    int value_store;
...
init(void) {
    ...
    if ((sem_x = sem_init(1)) < 0 || (sem_y = sem_init(1)) < 0 || (sem_z = sem_init(1)) < 0) {
        failed();
    }
